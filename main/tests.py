...
class Test(APITestCase):
    ...
    def test_1(self):
        ...
        self.assertEqual(response.data['rental'], 'rental-1')
        self.assertEqual(response.data['id'], 'Res-1')
    ...
    def test_3(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data['rental'], 'rental-1')
        self.assertEqual(response.data['id'], 'Res-3')
        self.assertEqual(response.data['check_in'], '2022-02-20')
        self.assertEqual(response.data['check_out'], '2022-03-10')
        self.assertEqual(response.data['previous_reservation'], 'Res-2')
    
    def test_4(self):
        response = self.client.get('/reservations/4', format='json')

        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data['rental'], 'rental-2')
        self.assertEqual(response.data['id'], 'Res-4')
    ...
    def test_5(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data['rental'], 'rental-2')
        self.assertEqual(response.data['id'], 'Res-5')
        self.assertEqual(response.data['check_in'], '2022-01-11')
        self.assertEqual(response.data['check_out'], '2022-01-20')
        self.assertEqual(response.data['previous_reservation'], 'Res-4')
